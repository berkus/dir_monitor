...
namespace asio {
    ...
    public:
        ...
        add_directory(std::string const& dirname)
        {
            ...
            if (wd == -1)
            {
                boost::system::system_error e(boost::system::error_code(errno,
                                              ...
                                              "boost::asio::dir_monitor_impl::add_directory: inotify_add_watch failed");
        ...
        void
        check_sub_directory(std::string const& dirname, bool add_sub_directory)
        {
    ...
    private:
        ...
        init_fd()
        {
            ...
            if (fd == -1)
            {
                boost::system::system_error e(boost::system::error_code(errno,
                                                                        boost::system::get_system_category()),
                                              "boost::asio::dir_monitor_impl::init_fd: init_inotify failed");
                boost::throw_exception(e);
            }
    ...
    public:
        ...
        begin_read()
        {
            stream_descriptor_->async_read_some(boost::asio::buffer(read_buffer_),
                                                boost::bind(&dir_monitor_impl::end_read,
                                                            this,
    ...
    private:
        void
        end_read(boost::system::error_code const& ec, std::size_t bytes_transferred)
        {
            if (!ec)
            {
                ...
                while (pending_read_buffer_.size() > sizeof(inotify_event))
                {
                    ...
                    switch (iev->mask)
                    {
                        case IN_CREATE: type = dir_monitor_event::added;
                            break;
                        case IN_DELETE: type = dir_monitor_event::removed;
                            break;
                        case IN_MODIFY: type = dir_monitor_event::modified;
                            break;
                        case IN_MOVED_FROM: type = dir_monitor_event::renamed_old_name;
                            break;
                        case IN_MOVED_TO: type = dir_monitor_event::renamed_new_name;
                            break;
