...
namespace helper {
    ...
    inline void
    throw_system_error_if(bool condition, std::string const& msg)
    {
        if (condition)
        {
    ...
    inline std::wstring
    to_utf16(const std::string& filename)
    {
        ...
        helper::throw_system_error_if(::MultiByteToWideChar(CP_UTF8, 0, filename.c_str(), -1, &buffer[0], size),
                                      "boost::asio::basic_dir_monitor_service::to_utf16: MultiByteToWideChar failed");
...
public:
    ...
    void
    add_directory(implementation_type& impl, const std::string& dirname)
    {
        std::wstring wdirname = helper::to_utf16(dirname);

        if (!boost::filesystem::is_directory(wdirname))
        {
            throw std::invalid_argument("boost::asio::basic_dir_monitor_service::add_directory: " +
                                        dirname +
                                        " is not a valid directory entry");
        }

        HANDLE handle = CreateFileW(wdirname.c_str(),
                                    FILE_LIST_DIRECTORY,
        ...
        helper::throw_system_error_if(INVALID_HANDLE_VALUE == handle,
                                      "boost::asio::basic_dir_monitor_service::add_directory: CreateFile failed");
        ...
        helper::throw_system_error_if(NULL == CreateIoCompletionPort(ck_holder->handle,
                                                                     iocp_,
                                                                     reinterpret_cast<ULONG_PTR>(ck_holder.get()),
                                                                     0),
                                      "boost::asio::basic_dir_monitor_service::add_directory: CreateIoCompletionPort failed");

        DWORD bytes_transferred; // ignored

        helper::throw_system_error_if(FALSE == ReadDirectoryChangesW(ck_holder->handle,
                                                                     ck_holder->buffer,
                                                                     sizeof(ck_holder->buffer),
                                                                     FALSE,
                                                                     0x1FF,
                                                                     &bytes_transferred,
                                                                     &ck_holder->overlapped,
                                                                     NULL),
    ...
    void
    remove_directory(implementation_type& impl, std::string const& dirname)
    {
    ...
    dir_monitor_event
    monitor(implementation_type& impl, boost::system::error_code& ec)
    {
    ...
    protected:
        void
        PostAndWait(boost::system::error_code const ec, dir_monitor_event const& ev) const
        {
    ...
    {
        async_monitor_io_service_.post(monitor_operation<Handler>(impl, get_io_service(), handler));
    }
...
private:
    ...
    init_iocp()
    {
        ...
        helper::throw_system_error_if(iocp == NULL,
                                      "boost::asio::basic_dir_monitor_service::init_iocp: CreateIoCompletionPort failed");
    ...
    work()
    {
        ...
        helper::throw_system_error_if(!GetQueuedCompletionStatus(iocp_,
                                                                 &bytes_transferred,
                                                                 reinterpret_cast<PULONG_PTR>(&ck),
                                                                 &overlapped,
                                                                 INFINITE),
        ...
        if (impl)
        {
            ...
            do
            {
                ...
                switch (fni->Action)
                {
                    case FILE_ACTION_ADDED: type = dir_monitor_event::added;
                        break;
                    case FILE_ACTION_REMOVED: type = dir_monitor_event::removed;
                        break;
                    case FILE_ACTION_MODIFIED: type = dir_monitor_event::modified;
                        break;
                    case FILE_ACTION_RENAMED_OLD_NAME: type = dir_monitor_event::renamed_old_name;
                        break;
                    case FILE_ACTION_RENAMED_NEW_NAME: type = dir_monitor_event::renamed_new_name;
                        break;
                }

                impl->pushback_event(dir_monitor_event(boost::filesystem::path(ck_holder->dirname)
                                                           / std::wstring(fni->FileName,
                                                                          fni->FileNameLength / sizeof(WCHAR)), type));
                offset += fni->NextEntryOffset;
            } while (fni->NextEntryOffset);

            ZeroMemory(&ck_holder->overlapped, sizeof(ck_holder->overlapped));
            helper::throw_system_error_if(!ReadDirectoryChangesW(ck_holder->handle,
                                                                 ck_holder->buffer,
                                                                 sizeof(ck_holder->buffer),
                                                                 FALSE,
                                                                 0x1FF,
    ...
    stop_work_thread()
    {
        ...
        helper::throw_system_error_if(!PostQueuedCompletionStatus(iocp_, 0, 0, NULL),
                                      "boost::asio::basic_dir_monitor_service::stop_work_thread: PostQueuedCompletionStatus failed");
    }
