...
struct splited_path
{
    splited_path(boost::filesystem::path const& p)
        : m_path(split_path(p)) {}

    splited_path(char const* p)
        : m_path(split_path(boost::filesystem::path(p))) {}

    friend bool
    operator==(splited_path const& lhs, splited_path const& rhs)
    {
    ...
    split_path(boost::filesystem::path const& p)
    {
